...
if (!function_exists('arrayCheck')) {
	...
	{
		return is_array($array) && isset($array[$key]) && $array[$key] !== 'null' && !empty($array[$key]);
	}
}
...
if (!function_exists('getFileLink')) {
    function getFileLink($size, $array, $offline = null)
    {
        ...
        if ($size != null && is_array($array)) {
            return static_asset('images/default/default-image-' . $size . '.png');
        } 

        return static_asset('frontend/img/icons/user-2.svg');
    }
}
...
if (!function_exists('countryCode')) {
    ...
    {
        if ($id) {
            $country = Cache::remember("country_code_{$id}", now()->addDay(), function () use ($id) {
                return Country::find($id);
            });
            $code = @$country->phonecode;
        } elseif (setting('default_country')) {
            $defaultCountryId = setting('default_country');
            $country = Cache::remember("country_code_{$defaultCountryId}", now()->addDay(), function () use ($defaultCountryId) {
                return Country::find($defaultCountryId);
            });
            $code = @$country->phonecode;
        } else {
            $country = Cache::remember("country_code_19", now()->addDay(), function () {
                return Country::find(19);
            });
            $code = @$country->phonecode;
        }

        if (!$code) {
            $code = '+1'; // Default fallback code
        }
