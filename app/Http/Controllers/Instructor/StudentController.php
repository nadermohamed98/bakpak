...
use App\DataTables\Instructor\StudentAssignmentsDataTable;
use App\DataTables\Instructor\StudentDataTable;
use App\DataTables\Instructor\StudentQuizzesDataTable;
...
class StudentController extends Controller
{
    ...
    public function assignments(StudentAssignmentsDataTable $dataTable,$userId){
        try {
            return $dataTable->with('user_id', $userId)->render('backend.instructor.student.assignments');
        } catch (\Exception $e) {
            Toastr::error($e->getMessage());
    ...
    public function quizzes(StudentQuizzesDataTable $dataTable, $userId, $courseId = null)
    {
        try {
            \Log::Info("CourseID => ", [$courseId ?? 'No Course ID provided']);
            ...
            $data = [
                'user' => $user,
                'courseId' => $courseId // This will be null if not passed
            ];
    
            // Pass both user_id and courseId to the DataTable (courseId will be null if not passed)
            return $dataTable->with(['user_id' => $userId, 'course_id' => $courseId])
                             ->render('backend.instructor.student.quizzes', $data);
