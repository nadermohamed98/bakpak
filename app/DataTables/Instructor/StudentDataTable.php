...
class StudentDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('assignments', function ($user) {
                return view('backend.instructor.student.iconShow', compact('user'));
            })

            ->addColumn('quizzes', function ($user) use ($courseId) {
                return view('backend.instructor.student.iconShowQuiz', compact('user', 'courseId'));
            })
    ...
    {
        $courseId = $this->course_id ?? $this->id;
        
        if ($courseId == null) {
            $coursesIds = Course::whereHas('users', function ($query) {
        ...
        return User::with('country')
            ->whereHas('checkout.enrolls', function ($query) use ($coursesIds) {
                $query->whereHas('enrollable', function ($query) {
                    $query->whereHas('users');
                })->where('enrollable_type', Course::class)->whereIn('enrollable_id', $coursesIds);
            })
