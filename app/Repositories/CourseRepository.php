...
class CourseRepository
{
	...
	public function activeCourses($data, $relation = [])
	{
		return Course::with($relation)->withCount('lessons')->withCount('enrolls')->withAvg('reviews', 'rating')
			...
			->when(arrayCheck('suggested_course', $data), function ($query) {
			...
			})->when((isset($data['exclude_bookshelves']) && $data['exclude_bookshelves'] === true),
				function ($query) {
					$query->where('course_type', '!=', 'book_shelf');
				...
				$query->where('is_free', 0)->where('discount', '>', 0)->where('discount_start_at',
					'<',
					now())->where('discount_end_at', '>=', now());
				...
				$query->withSum('progresses', 'progress')->whereHas('enrolls.checkout', function ($query) use ($data) {
					$query->where('status', 1)->where('user_id', $data['user_id'])->when(arrayCheck('course_view',
						$data),
				...
				$query->where('category_id', $data['category_id'])->when(arrayCheck('with_child_category', $data),
					function ($query) use ($data) {
						$query->orWhereHas('category', function ($query) use ($data) {
							$query->where('parent_id', $data['category_id']);
						});
					});
			})->when(arrayCheck('related', $data) && arrayCheck('id', $data) && arrayCheck('category_id', $data),
				function ($query) use ($data) {
				...
				$filter = $data['filter'];
				$query->$filter();
			})->when(arrayCheck('category_ids', $data) && count($data['category_ids']) > 0,
				function ($query) use ($data) {
					$query->whereIn('category_id', $data['category_ids'])->when(arrayCheck('with_child_category',
						$data),
						function ($query) use ($data) {
							$query->orWhereHas('category', function ($query) use ($data) {
				...
				})->when(arrayCheck('level_ids', $data) && count($data['level_ids']) > 0,
				function ($query) use ($data) {
					$query->whereIn('level_id', $data['level_ids']);
				})->when(arrayCheck('subject_ids', $data) && count($data['subject_ids']) > 0,
				function ($query) use ($data) {
					$query->whereIn('subject_id', $data['subject_ids']);
				})->when(arrayCheck('course_type', $data) && count($data['course_type']) > 0,
				function ($query) use ($data) {
					$query->whereIn('course_type', $data['course_type']);
    ...
    public function activeCoursesIDs($data, $relation = [])
    {
        $courseIds = Course::with($relation)
            ...
            ->where('is_private', 0)
            ->whereIn('course_type', ['course','live_class'])
            ->when(arrayCheck('my_course', $data), function ($query) use ($data) {
                $query->whereHas('enrolls.checkout', function ($query) use ($data) {
                    $query->where('user_id', $data['user_id'])
    ...
    public function getBookShelves(){
        return Course::where('course_type','book_shelf')->get();
    }
}