                ...
                <div class="header-top d-flex justify-content-between align-items-center">
                    ...
                    @if(request()->routeIs('courses.index'))
                        <div class="oftions-content-right mb-12">
                            ...
                            @endif
                            @if(hasPermission('courses.create') && count($book_shelfs) > 1)
                                <div class="d-flex justify-content-between align-items-center">
                                    <button type="button"
                                            data-bs-toggle="modal"
                                            data-bs-target="#book_shelfs"
                                            class="btn sg-btn-primary add_modal">
                                        <i class="las la-plus"></i> {{ __('clone_from_book_shelf') }}</button>
                                </div>
                                @include('backend.admin.course.book-shelfs')
                            @endif
                        </div>
                    @endif
                </div>
                                        ...
                                        <div class="col-custom">
                                            <div class="multi-select-v2">
                                                ...
                                                <select id="ins_by_org"
                                                        ...
                                                        aria-label=".form-select-lg example">
                                                    <option value=""
                                                            selected>{{ __('select_organization') }}</option>
                                                    @if($organization)
                                            ...
                                            <div class="select-type-v2">
                                                ...
                                                <select id="instructor_ids"
                                                    ...
                                                    @foreach($instructors as $instructor)
                                                        <option value="{{ $instructor->id }}" {{ $instructor_ids && in_array($instructor->id,$instructor_ids) ? 'selected' : '' }}>{{ $instructor->name }}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-custom">
                                            <div class="select-type-v2">
                                                ...
                                                <select id="courseStatus"
                                                    ...
                                                    <option value="IN_REVIEW" {{ $status == "IN_REVIEW" ? 'selected' : '' }}>{{ __('in_review') }}</option>
