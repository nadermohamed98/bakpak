                                ...
                                @foreach($quiz->questions as $key => $question)
                                    @if($question->question_type == 'default')
                                        <div class="quiz-question">
                                            <h4 class="question-title" id="{{$question->id}}">{!! $question->question !!}</h4>
                                                ...
                                                @foreach($question->answers as  $key2 =>$answer)
                                                    ...
                                                    <div class="option">
                                                        <input type="radio" name="student_answer_{{$question->id}}"
                                                               id="op-{{ $answer['answer']  }}_{{ $key2  }}_{{$key}}"
                                                               value="{{$key2+1}}">
                                                        <label
                                                            for="op-{{ $answer['answer']  }}_{{ $key2  }}_{{$key}}">{{ $answer['answer'] }}</label>
                                                    </div>
                                                @endforeach
                                                ...
                                                @foreach($question->answers as  $key3 =>$answer)
                                                    ...
                                                    <div class="option">
                                                        <input type="checkbox" name="student_answer_{{$question->id}}[]"
                                                               id="op-{{ $answer['answer']  }}_{{ $key3  }}_{{$key}}"
                                                               value="{{$key3+1}}">
                                                        <label
                                                            for="op-{{ $answer['answer']  }}_{{ $key3  }}_{{$key}}">{{ $answer['answer'] }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
