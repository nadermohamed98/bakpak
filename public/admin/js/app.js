...
(function ($) {
	...
	$(document).ready(function () {
		...
		$(document).on("click", ".edit_modal_book_shelf", function () {
			let id = $(this).attr("data-modal");
			modal_id = $("#" + id);
			modal_id.modal("show");
			modal_id.find(".create_sub_title").addClass("d-none");
			...
			$.ajax({
				...
				success: function (response) {
					if (response.html) {
						modal_id.find(".form_div_book_shelf").html(response.html);
						defaultEditor();
						selectionFields();
					} else {
						let keys = Object.keys(response);
		...
		$(document).on("change click", "#section_id", function () {
			let $this = $(this); // Store reference to $(this)
			let value = $(this).val();
			console.log($(this));
			let url = $(this).attr("data-url");
			let active = $(this).attr("data-active") ?? 1;
			if (value) {
				$.ajax({
					type: "GET",
					url: url,
					...
					success: function (response) {
						if (response.error) {
							...
							var modal = $this.closest('.modal'); // Use $this here
							// Reinitialize Select2 inside the modal
							reInitializeSelect(modal.find("#lesson_id"), response);
						}
					},
					error: function (response) {
						toastr.error(response.responseJSON.error);
...
function searchCourse(el) {
    ...
    let excludeBookshelves = el.data("exclude_bookshelves") ? el.data("exclude_bookshelves") : 0;
    let fields = {
...
function searchLessons(el) {
	...
	let active = el.data("active") ? el.data("active") : 'true'
	el.select2(select2Options(el, route, {'active': active}));
}
